...
%% Функции
%is_queue(any) -> boolean - возвращает то, является ли any очередью
%empty() -> queue - создаёт пустую очередь
%insert(Key, Value) -> queue - вставляет эл-т в овередь 
%extract_minimum() -> {Key, Value} | {'error'} - удаляет из очереди эл-т с наименьшим приоритетом
